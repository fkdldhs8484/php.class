                ...
                <form action="adminJoinSave.php" name="join" method="post" onsubmit="return joinChecks()">
                    <fieldset>
                        ...
                        <div class="join__box">
                            ...
                            <div>
                                <label for="youBitrh">생년월일</label>
                                <input type="text" id="youBitrh" name="youBitrh" placeholder="YYY-MM-DD" maxlength="15" required>
                                <p class="msg" id="youBitrhComment"><!--* 생년월일이 일치하지 않습니다.--></p>
                            </div>
                            <div>
                                <label for="youPhone">휴대폰번호</label>
                                <input type="text" id="youPhone" name="youPhone" placeholder="010-0000-0000" required>
    ...
    <script>
        ...
        function joinChecks(){
            ...
            if(!getYouNickName.test($("#YouNickName").val())){
                $("#youNickNameComment").text("닉네임은 한글 또는 숫자만 사용가능입니다.");
                $("#youNickName").val("");
                return false;
            }
            ...
            let getTouPassNum = getYouPass.search(/[0-9]/g);
            let getTouPassEng = getYouPass.search(/[a-z]/ig);
            let getYouPassSpe = getPass.search(/[`~!@@#$%^&*|₩₩₩'₩";:₩/?]/gi);

            if(getYouPass.length < 8 || getYouPass.length > 20){
                $("#youPassCommnet").text("8자리 ~ 20자리 이내로 입력해주세요!");
                return false;
            }else if (getYouPass.search(/\s/) != -1){
                $("#youPassCommnet").text("비밀번호는 공백없이 입력해주세요!");
                return false;
            }else if(getYouPassNum < 0 || getTouPassEng < 0 || getYouPassSpe < 0){
                $("#youPassCommnet").text("영문, 숫자, 특수문자를 혼합하여 입력해주세요!");
                return false;
            }
            ...
            if($("#youPass").val() !== $("#youPassC").val()){
                $("#youPassCCommnet").text("비밀번호가 동일하지 않습니다!");
                return false;
            }
            ...
            if(!getYouName.test($("#YouName").val())){
                $("#youNameComment").text("이름은 한글로만 가용 가능합니다!");
                $("#youName").val("");
                return false;
            }
            ...
            // 생년월일 공백 확인
            if($("#youBitrh").val() == ""){
                $("#youBitrhComment").text("생년월일 입력해주세요!");
                return false;
            }

            // 생년월일 유효성 검사
            let getYouBitrh = RegExp(/^(19[0-9][0-9]|20\d{2})-(0[0-9]|1[0-2])-(0[1-9]|[1-2][0-9]|3[0-1])$/);
            if(!getYouBitrh.test($("#youBitrh").val())){
                $("#youBitrhComment").text("생년월일이 정확하지 않습니다. 올바른 생년월일(YYYY-MM-DD)을 적어주세요!");
                return false;
            }
